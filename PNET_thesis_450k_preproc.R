...
keep=grep('pancreas', pData(RGset)$tumor_in_specimen, ignore.case=TRUE)
RGset=RGset[,keep]
...
#load("/athena/masonlab/scratch/users/nai2008/PNET_thesisProject/DNAm_analysis/rdas/RGset_Chan_controls.rda") # annotation: ilmn12.hg19; 23 samples
...
GRset # 485512 probes; 87 samples

GRset=dropLociWithSnps_mine(object=GRset, maf=0, snpAnno = "SNPs.147CommonSingle") # 468392 probes (17120 SNP probes dropped)

# drop sex chr
anno=getAnnotation(GRset)
GRset=GRset[! anno$chr %in% c("chrX","chrY"),] # 456928 probes (11464 sex chr probes dropped)
